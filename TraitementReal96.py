...
def traitementDonneesReelles96(fileExcel,listeFeuilles, mixtures_dict, column_nucleotypes_dict):
    ...
    for feuilleNum in listeFeuilles:
        ...
        min_NMLog = scipy.optimize.minimize(likelihoodLog,
                                            lam_init,
                                            args=(reads,G_T),
                                            method= 'Nelder-Mead',
                                            bounds=bounds,
                                            options={"maxiter":10,"maxfev":500})
        print(min_NMLog)

        cons = ({'type': 'eq', 'fun': lambda x: np.sum(x) - 1})
        print("lancement SLSQP")
...
traitementDonneesReelles96("outputVrai96.xlsx",[7], mixtures_dict, column_nucleotypes_dict)
