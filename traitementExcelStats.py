...
for f in range(1,21):
    data = pd.read_excel("output.xlsx","N°"+str(f))
    # data = pd.read_excel("output.xlsx","N°2")
    # print(data)
    ...
    min_NM = scipy.optimize.minimize(likelihood,lam_init,args=(reads,G),method= 'Nelder-Mead',bounds=bounds,options={'maxiter':5000,'maxfev':5000})
    min_NMLog = scipy.optimize.minimize(likelihoodLog,lam_init,args=(reads,G),method= 'Nelder-Mead',bounds=bounds,options={'maxiter':5000,'maxfev':5000})
    ...
    ## Test n°1 : quels algos convergent vers la même solution ?
    ...
    # memes_algo = np.array(memes_algo,dtype="object")
    ...
    def rechercheAlgos(valeur):
        for element in memes_algo_concat:
            if lambdas[element[i]] == valeur:
                return element
        return "La valeur demandée n'est pas un résultat d'un algorithme"
    ...
    print(df)
    df.to_excel(writer,"N°"+str(f),startcol=13,index=False)
    print(matrice)
    ...
    print(df2)
    df2.to_excel(writer,"N°"+str(f), startrow=13,startcol=13)

    writer.save()



## Test n°2 : Quel est le meilleur algorithme
